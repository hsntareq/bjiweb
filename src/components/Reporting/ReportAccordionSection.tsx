...
export const ReportAccordionSection: React.FC<ReportAccordionSectionProps> = ({ 
  ...
  return (
    <div className={`group transition-all duration-300 bg-white rounded-3xl border ${isOpen ? 'border-indigo-100 shadow-xl shadow-indigo-500/5' : 'border-gray-100 shadow-sm hover:shadow-md'} overflow-hidden mb-6`}>
      <div 
        className={`px-8 py-6 flex items-center justify-between cursor-pointer transition-colors ${isOpen ? 'bg-gradient-to-r from-indigo-50/50 to-white' : 'hover:bg-gray-50/50'}`}
        onClick={() => setIsOpen(!isOpen)}
      >
        <div className="flex items-center gap-4">
          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-300 ${isOpen ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30' : 'bg-gray-100 text-gray-500 group-hover:bg-gray-200'}`}>
            <Icon className="w-6 h-6" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h3 className={`font-bold transition-colors text-xl ${isOpen ? 'text-gray-900' : 'text-gray-700'}`}>{title}</h3>
              {completed && <CheckCircle2 className="w-5 h-5 text-emerald-500" />}
            </div>
            {!isOpen && <p className="text-sm text-gray-400 font-medium">Click to expand details</p>}
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          {onEdit && (
            <button 
              ...
              className="flex items-center gap-2 px-5 py-2.5 bg-white text-indigo-600 border border-indigo-100 rounded-xl hover:bg-indigo-600 hover:text-white transition-all text-sm font-bold shadow-sm active:scale-95"
            >
