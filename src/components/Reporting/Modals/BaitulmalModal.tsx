          ...
          <div className="grid grid-cols-2 gap-8">
             <div className="space-y-1">
                ...
                {incomeItems.map(item => (
                   <div key={item.id} className="flex justify-between items-center border-b border-gray-100 pb-1">
                      <span>{item.label}</span>
                      <input type="number" value={data.income?.[item.id] || 0} onChange={(e) => handleIncomeChange(item.id, parseInt(e.target.value) || 0)} className="w-20 border rounded p-1 text-right bg-white" />
                   </div>
                ))}
                <div className="flex justify-between items-center pt-4 font-bold text-emerald-700">
                   <span>মোট আয় =</span>
                   <span>{data.income?.totalIncome || 0} /=</span>
                </div>
                <div className="flex justify-between items-center pb-1">
                   <span>গত মাসের উদ্বৃত্ত =</span>
                   <input type="number" value={data.income?.previousMonthSurplus || 0} onChange={(e) => handleIncomeChange('previousMonthSurplus', parseInt(e.target.value) || 0)} className="w-20 border rounded p-1 text-right bg-white" />
                </div>
                <div className="flex justify-between items-center font-bold bg-emerald-50 p-2 rounded border border-emerald-100">
             ...
             <div className="space-y-1">
                ...
                {expenseItems.map(item => (
                   <div key={item.id} className="flex justify-between items-center border-b border-gray-100 pb-1">
                      <span>{item.label}</span>
                      <input type="number" value={data.expense?.[item.id] || 0} onChange={(e) => handleExpenseChange(item.id, parseInt(e.target.value) || 0)} className="w-20 border rounded p-1 text-right bg-white" />
                   </div>
                ))}
                <div className="flex justify-between items-center pt-4 font-bold text-red-700">
                   <span>মোট ব্যয় =</span>
                ...
                <div className="flex justify-between items-center pb-1 font-medium text-blue-600">
                   ...
                   <span className="w-20 text-right pr-2 font-bold">{data.expense?.monthlySurplus || 0}</span>
                </div>
                <div className="flex justify-between items-center font-bold bg-red-50 p-2 rounded border border-red-100">
